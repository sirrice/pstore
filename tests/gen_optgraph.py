...
def get_overheads(eids):
    q = """SELECT sum(opcost), sum(disk)
    FROM pstore_overhead as po, workflow_run as wr, exec
    WHERE po.wid = wr.rowid and wr.eid = exec.rowid and exec.rowid = ?"""
    disks = []
    opcosts = []
    for eid in eids:
        cur.execute(q, (eid,))
        opcost, disk = tuple(cur.fetchone())
