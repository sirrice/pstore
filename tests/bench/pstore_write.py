...
def run_op(arr, strat, noutput, fanin, oclustsize, density):
    ...
    rpstore = None
    for i in xrange(3):
        if rpstore: rpstore.close()
        
        op = BenchOp(arr, strat, noutput, fanin, oclustsize, density)
        Runtime.instance().set_strategy(op, strat)
        start = time.time()
        op.wrapper._arr = arr
        fprefix, pstore = op.run([arr], 0)
        opcost = (time.time() - start) - pstore.stats.get('write', 0)
        

        if i == 0: continue
        runcosts = (pstore.stats.get('_serialize',0),
                    0, 
                    pstore.stats.get('write', -1),
                    opcost,
                    pstore.disk())
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        runid = add_run(conn, inputsize, shape, notes='ptr_model')
    print "runid: ", runid
